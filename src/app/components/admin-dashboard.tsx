...
// Room definitions
const ROOMS = [
  { id: 0, name: "Lobby", short: "Lobby", color: "text-gray-400", bg: "bg-gray-500/10", border: "border-gray-500/30", dot: "bg-gray-400" },
  { id: 1, name: "Room of Knowledge", short: "Knowledge", color: "text-teal-400", bg: "bg-teal-500/10", border: "border-teal-500/30", dot: "bg-teal-400" },
  { id: 2, name: "Room of Abstracts", short: "Abstracts", color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/30", dot: "bg-blue-400" },
  { id: 3, name: "Room of Analytics", short: "Analytics", color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/30", dot: "bg-purple-400" },
  { id: 4, name: "Room of Sciencebattle", short: "Sciencebattle", color: "text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/30", dot: "bg-orange-400" },
  { id: 5, name: "Final Stage", short: "Final", color: "text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-500/30", dot: "bg-yellow-400" },
  { id: 6, name: "Completed", short: "Done", color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/30", dot: "bg-green-400" },
];
...
const MOCK_ACTIVE_TEAMS: ActiveTeam[] = [
  { id: "t1", name: "The Nightingales", mission: "The Silent Symptom", currentRoom: 3, startedAt: "14:30", membersCount: 4, score: 2200, timeRemaining: 45, timedOut: false },
  { id: "t2", name: "Code Blue Crew", mission: "The Silent Symptom", currentRoom: 1, startedAt: "14:45", membersCount: 3, score: 800, timeRemaining: 1920, timedOut: false },
  { id: "t3", name: "EBN Warriors", mission: "The Silent Symptom", currentRoom: 4, startedAt: "14:15", membersCount: 5, score: 3100, timeRemaining: 25, timedOut: false },
  { id: "t4", name: "The Evidence Squad", mission: "The Silent Symptom", currentRoom: 2, startedAt: "14:35", membersCount: 4, score: 1500, timeRemaining: 1380, timedOut: false },
  { id: "t5", name: "Clinical Detectives", mission: "The Silent Symptom", currentRoom: 5, startedAt: "13:50", membersCount: 3, score: 3800, timeRemaining: 480, timedOut: false },
  { id: "t6", name: "Stat Nurses", mission: "The Silent Symptom", currentRoom: 1, startedAt: "15:00", membersCount: 4, score: 400, timeRemaining: 2700, timedOut: false },
  { id: "t7", name: "Research Rebels", mission: "The Silent Symptom", currentRoom: 2, startedAt: "14:40", membersCount: 3, score: 1200, timeRemaining: 1080, timedOut: false },
];

// Mock past games
const MOCK_PAST_GAMES: PastGame[] = [
  {
    id: "g1",
    date: "2026-03-02",
...
type SortField = "date" | "mission" | "winner" | "teams" | "time" | "score";
...
function getTimeColor(seconds: number, timedOut: boolean): string {
...
export function AdminDashboard({ onBack }: AdminDashboardProps) {
  ...
  const filteredGames = MOCK_PAST_GAMES.filter(
    ...
    switch (sortField) {
      ...
      case "mission": return dir * a.mission.localeCompare(b.mission);
      case "winner": return dir * a.winnerTeam.localeCompare(b.winnerTeam);
      case "teams": return dir * (a.totalTeams - b.totalTeams);
      case "time": return dir * a.winnerTime.localeCompare(b.winnerTime);
      case "score": return dir * (a.winnerScore - b.winnerScore);
      default: return 0;
    }
  });
  ...
  const teamsInProgress = activeTeams.filter((t) => t.currentRoom > 0 && t.currentRoom < 6 && !t.timedOut).length;
      ...
      <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-5">
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500 pl-10">
                    ...
                    <span className="text-red-400 font-[JetBrains_Mono,monospace]">
                      {extendModalTeam.timedOut ? "TIME'S UP" : formatTime(extendModalTeam.timeRemaining)}
                    </span>
                  </div>
                </div>
        ...
        <div className="flex-1 px-6 md:px-10 py-6 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { icon: Users, label: "Active Teams", value: totalActiveTeams, color: "text-teal-400", bg: "bg-teal-500/10", border: "border-teal-500/20" },
              { icon: Activity, label: "In Progress", value: teamsInProgress, color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
              { icon: AlertTriangle, label: "Timed Out", value: teamsTimedOut, color: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/20" },
              { icon: Trophy, label: "Completed", value: teamsCompleted, color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/20" },
            ].map((stat) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 10 }}
          ...
          <div className="flex items-center gap-1 mb-6 bg-white/5 rounded-xl p-1 w-fit">
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-2 ${
                activeTab === "monitor" ? "bg-teal-500 text-white" : "text-gray-400 hover:text-white hover:bg-white/5"
              }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-2 ${
                activeTab === "history" ? "bg-teal-500 text-white" : "text-gray-400 hover:text-white hover:bg-white/5"
              }`}
            >
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/5">
                        <th className="px-6 py-3 text-left text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">TEAM</th>
                        <th className="px-6 py-3 text-left text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">CURRENT ROOM</th>
                        <th className="px-6 py-3 text-left text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">PROGRESS</th>
                        <th className="px-6 py-3 text-left text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">TIME LEFT</th>
                        <th className="px-6 py-3 text-left text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">STARTED</th>
                        <th className="px-6 py-3 text-right text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">SCORE</th>
                        <th className="px-6 py-3 text-center text-gray-500 text-xs font-[JetBrains_Mono,monospace] tracking-wider">ACTIONS</th>
                      </tr>
                    </thead>
                    <tbody>
                      {activeTeams
                        ...
                        .map((team) => {
                          ...
                          return (
                            ...
                            >
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                                    ...
                                    <span className={`text-sm font-[JetBrains_Mono,monospace] ${
                                  ...
                                  <div>
                                    ...
                                    <span className="text-gray-600 text-[10px]">{team.membersCount} members</span>
                                  </div>
                                </div>
                              </td>
                              <td className="px-6 py-4">
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2">
                                  <Timer className={`w-3.5 h-3.5 ${timeColor} ${team.timedOut ? "animate-pulse" : ""}`} />
                                  <span className={`text-sm font-[JetBrains_Mono,monospace] ${timeColor} ${
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="flex items-center gap-2">
                              <Timer className={`w-3 h-3 ${timeColor} ${team.timedOut ? "animate-pulse" : ""}`} />
                              <span className={`text-xs font-[JetBrains_Mono,monospace] ${timeColor}`}>
                                {team.timedOut ? "TIME'S UP" : formatTime(team.timeRemaining)}
                              </span>
                            </div>
                          </div>
