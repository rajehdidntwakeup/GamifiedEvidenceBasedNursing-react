...
export function RoomOfKnowledge({ mission, onBack, onProceedToRoom2 }: RoomOfKnowledgeProps) {
  ...
  const handleSelectAnswer = async (index: number) => {
    ...
    const isCorrect = await roomOfKnowledgeApi.verifyAnswer(roomId, activeQuestion.id, answerId)
  ...
  const progressPercent =
    questions.length > 0 ? ((currentQuestion + (isAnswered ? 1 : 0)) / questions.length) * 100 : 0
          ...
          >
            ...
            <div className='bg-white/5 border border-white/10 rounded-2xl p-6 mb-8'>
              ...
              <div className='flex items-center justify-between text-sm'>
                <span className='text-gray-500'>{passed ? 'Perfect score!' : 'Not enough'}</span>
                  ...
                  >
                    ...
                    <div className='flex-1 min-w-0'>
                      <p className='text-gray-300 text-sm leading-relaxed'>
                        <span className='text-gray-500 font-[JetBrains_Mono,monospace] mr-2'>
                          #{i + 1}
                        </span>
                        {result.question}
                      </p>
          ...
          <div className='max-w-3xl w-full'>
            ...
            <AnimatePresence mode='wait'>
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <h2 className='text-xl md:text-2xl text-white mb-8 tracking-tight'>{q.question}</h2>
                ...
                <div className='space-y-3 mb-8'>
                  {q.options.map((option, i) => {
                    ...
                    const isCorrect =
                      verifiedCorrectIndex !== null
                        ? i === verifiedCorrectIndex
                        : i === q.correctIndex
