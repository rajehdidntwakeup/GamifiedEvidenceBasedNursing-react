...
export function RoomOfAbstracts({ onBack, onProceedToRoom3 }: RoomOfAbstractsProps) {
  ...
  const updateCell = (rowIndex: number, field: keyof TableRow, answerId: number | null) => {
    setTableRows((prev) => {
      ...
      updated[rowIndex] = {
        ...updated[rowIndex],
        [field]: { ...updated[rowIndex][field], answerId },
      }
      return updated
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const answers = tableRows
        ...
        .filter(
          (a): a is { questionId: number; answerId: number | null } =>
            a !== null && a.answerId !== null,
        )
          ...
          >
            ...
            <div className='bg-white/5 border border-white/10 rounded-2xl p-6 mb-8'>
              <div className='flex items-center justify-between mb-4'>
                ...
                <span className='text-white text-2xl font-[JetBrains_Mono,monospace]'>{pct}%</span>
              </div>
              ...
              <div className='flex items-center justify-between text-sm mt-4'>
                <span className='text-gray-500'>{didPass ? 'Perfect score!' : 'Not enough'}</span>
              ...
              >
                ...
                <div className='relative z-10'>
                  ...
                  <p className='text-gray-600 text-xs mt-4 font-[JetBrains_Mono,monospace]'>
                    FRAGMENT 2 OF ? // COLLECTED:{' '}
                    {previousKey ? previousKey.replace('-', ', ') + ', ' : ''}
                    {resultKey.replace('-', ', ')}
                  </p>
                </div>
              </motion.div>
        ...
        <div className='flex-1 px-6 py-8 overflow-y-auto'>
          <div className='max-w-5xl mx-auto'>
            ...
            >
              ...
              <p className='text-gray-400 max-w-2xl mx-auto text-sm'>{data.mainQuestion}</p>
            </motion.div>
                ...
                <table className='w-full text-sm'>
                  <thead>
                    <tr className='border-b border-white/10 bg-white/[0.02]'>
                      <th className='py-3 px-4 text-left text-gray-500 font-[JetBrains_Mono,monospace] text-xs w-8'>
                        #
                      </th>
                      <th className='py-3 px-4 text-left text-gray-500 font-[JetBrains_Mono,monospace] text-xs'>
                        Study Title & Author
                      </th>
                      <th className='py-3 px-4 text-left text-gray-500 font-[JetBrains_Mono,monospace] text-xs'>
                        Pyramid (LoE)
                    ...
                    {tableRows.map((row, i) => (
                      <tr key={i} className='border-b border-white/5'>
                        ...
                        <td className='py-4 px-4'>
                          ...
                          >
                            <option value='' className='bg-[#0a1f22] text-white'>
                              Select author...
                            </option>
                        ...
                        <td className='py-4 px-4'>
                          <select
                            ...
                            onChange={(e) =>
                              updateCell(i, 'ahcpr', e.target.value ? Number(e.target.value) : null)
                            }
                            className='w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500 focus:outline-none text-sm appearance-none cursor-pointer'
                            style={{
                              backgroundImage: `url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='%236b7280' stroke-width='2'%3E%3Cpath d='m6 9 6 6 6-6'/%3E%3C/svg%3E")`,
