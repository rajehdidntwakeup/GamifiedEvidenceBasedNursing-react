    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className={`w-10 h-10 rounded-xl ${mission.bgColor} ${mission.borderColor} border flex items-center justify-center`}>
            {Icon && <Icon className={`w-5 h-5 ${mission.textColor}`} />}
          </div>
          <span className={`font-[JetBrains_Mono,monospace] text-xs ${mission.textColor}`}>{mission.subtitle}</span>
        </div>
        ...
        {/* Arrow */}
        <div className={`flex items-center gap-1 ${mission.textColor} opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0`}>
